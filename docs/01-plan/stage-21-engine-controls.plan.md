...
> **상태: 완료(2026-08-03) — 표적 재검토 통과(DoD 자동 검증 6/6·치명 0·중요 0) + 사용자 이행 항목(DoD 7 모델 실호출)도 2026-08-03 사용자 확인으로 종결.**
...
- [x] **모델 실호출 확인(사용자 이행 — 실 LLM 소비용)**: 엔진별 선택 모델로 소형 변환 각 1회 — 모델 인자가 실제 반영되는지(로그·usage로 확인). — **2026-08-03 사용자 확인 완료**: claude-cli = `--model sonnet` 플래그 실전달 확인 · claude-api = Haiku 4.5(`claude-haiku-4-5-20251001`) usage 확인. codex-cli는 모델 소목록 빈 배열(D4 보수 규칙)이라 확인 대상 아님.
...
- **2026-08-03 DoD 7(모델 실호출) 사용자 확인 — stage 21 완전 종결**: 실기동(uvicorn :8000, 재빌드 dist)에서 엔진별 선택 모델 소형 변환 각 1회 수행 — **claude-cli**: 서버 로그에 `--model sonnet` 플래그 실전달 확인 · **claude-api**: 선택 모델 Haiku 4.5(`claude-haiku-4-5-20251001`)가 usage에 반영됨 확인. codex-cli는 모델 소목록 빈 배열이라 대상 외. 이로써 DoD 7/7 전건 충족 — M21(F47) 완료.
