...
- [ ] 계획서 F46에 구현 확정 사항 기록(잠금 구간 최종 범위·표본 점검 결과) · 설계 §4.22에 구현 중 보완 반영(어긋나면 착수 중단 후 보고 — DDL·DB 쓰기는 특히) · 설계 screens(§5.11 또는 제안함 절)에 [반입 개선] 탭 블록 반영
- [ ] 사용자 매뉴얼(F39): 반입 개선 루프 사용법(실패 사례가 쌓이는 곳·제안 생성 비용 확인·승인/거절·정책 잠금 의미·회귀 재검증·코드 수정 필요 제안의 인계 방법) 추가
- [ ] 이 문서 체크박스 갱신(불변 규칙 10) · CLAUDE.md 문서 지도 갱신(4.1~4.22·[S20]·stage 20 표기)은 오케스트레이터 담당
