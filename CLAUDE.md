...
| 문서 | 역할 |
|---|---|
| `docs/01-plan/study-app.plan.md` | 마스터 계획 — 기능 F01~F41, **스키마 DDL 단일 출처(§6.2)**, 로드맵 M1~M15, 리스크 R1~R18 |
| `docs/02-design/study-app.design.md` | API 명세(엔드포인트에 [S1]~[S14] 태그) + 화면 상세 12개 + 토큰/상태관리 |
| `docs/01-plan/stage-{1..15}-*.plan.md` | 단계별 작업 지시서 — 체크리스트·DoD·제외 범위. MVP = stage 1~3, v1 = ~7, v1.x = 8~11, 후속 = 12 (**stage 1~12 완료** — 2026-07-26). **stage 13**(사설 어댑터 제거 + 수동 반입 UX) · **stage 14**(큐넷 오픈API, 후순위)는 계획 확정·미착수. **stage 15**(멀티 벤더 LLM 엔진 — Codex CLI)는 계획 등록·**착수 게이트 있음**(M13·M14 종료 + 품질 재실험 — 문서 상단 게이트 참조) |
