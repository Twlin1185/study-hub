...
| `docs/01-plan/stage-{1..19}-*.plan.md` | 단계별 작업 지시서 — 체크리스트·DoD·제외 범위. **stage 1~19 완료**(17~19는 2026-08-02 검토 통과 — 18은 라이브 완주·표본 검증(R21) 사용자 확인 3건 잔여, 19는 라이브 완주·표본 검증(R22) 사용자 확인 2건 잔여(검토 경미 6건은 해소), 각 stage 문서 완료 기록 참조) · **20**(반입 자기 개선 루프 F46, M20)은 계획서 등재만(순서 자유). 각 단계의 완료 경위·게이트 기록은 해당 stage 문서 참조 |
