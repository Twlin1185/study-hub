...
- **로그인**: 자격증명은 전역 `~/.claude`(`.credentials.json`) 공유 — 다른 곳에서 로그인했으면 격리본도 즉시 통과(실측). 미로그인이면 **④-3(S42-B6)이 개정** — 앱이 [로그인]으로 `claude auth login` 창을 서버 PC에 띄운다(자격증명은 여전히 CLI가 관리 — 앱은 만지지 않는다). 종전 문장 "앱이 로그인을 대행하지 않는다"는 ④-3으로 대체.
- **프라이버시 고지**: 변환 원문이 `~/.claude` 세션 기록에 남는다 — 카드에 고정 노출(codex `~/.codex` 고지와 대칭).

**④-3 CLI 원클릭 온보딩 — 설치 인식·로그인 실행 (S42-B6 추기 — 2026-08-29)**

| 메서드/경로 | 설명 |
|---|---|
| `POST /api/llm/engines/{id}/login` | CLI형·installable 엔진만(그 외 422). 실행 파일 미발견 = 422(`detail.reason:'not_installed'` — 프론트는 설치 단계로 복귀). 강제 진단으로 이미 로그인 = `{status:'already_logged_in'}` · 같은 엔진의 로그인 프로세스가 살아 있음 = `{status:'in_progress'}` · 아니면 **서버 PC에 새 콘솔 창**으로 `claude auth login` / `codex login`을 띄운다(CLI가 브라우저 로그인 페이지를 스스로 연다 · 앱은 자격증명을 만지지 않는다) → `{status:'started'}`. 창을 못 열면 502. 프로세스 종료 시 서버가 진단 캐시를 무효화 |
| `GET /api/llm/status` | `engines[]` += `login_pending: bool|null`(CLI형만 — 로그인 프로세스 생존 여부. 순수 추가) |

- **탐색 순서 개정(②·④ 공통)**: 격리본(`tools/…`) → 프로세스 PATH → **레지스트리 현재 PATH(HKCU·HKLM)** → **잘 알려진 설치 폴더**(claude: `~/.local/bin`·npm 전역 bin, codex: npm 전역 bin). 근거: 실행 중인 서버의 PATH는 시작 시점에 고정돼 외부 설치기가 넣은 CLI를 못 봤고 사용자가 서버를 재시작해야 했다(실사용 보고). 공용 `services/exe_locate.py`, 캐시 없음(진단 TTL 60초가 상한).
- **화면 계약(§5.11)**: 카드 = 3단계 스테퍼 "① 설치 → ② 로그인 → ③ 완료". [설치] 성공 → `?refresh=1` 재진단 → 미로그인이면 **자동으로 login 호출** → "이 PC에 로그인 창이 열렸습니다 …" + 4초 폴링(`?refresh=1`, 최대 5분) → 로그인 확인 시 "✓ 설정 완료". 폴링은 로그인 단계에서만(평시 강제 진단 금지 원칙 유지). 한계 명시: 로그인 창·브라우저는 **서버가 실행 중인 PC**에 열린다(OAuth 콜백이 그 PC의 localhost로 오므로 폰에서는 완료 불가 — 카드에 1줄 고지).
...
- **(S42-B6 개정 — 2026-08-29)**: 아래 ⓑ "앱이 대화형 로그인을 대행하지 않는다"는 **④-3으로 개정** — [로그인] 버튼이 서버 PC에 `codex login` 창을 띄우고 카드가 완료를 자동 인식한다. ⓒ의 "버전" 표시는 스테퍼 개편으로 **삭제**(status에 version 필드 없음 — 설치 응답의 version만 존재) → ⓒ = 설치·로그인 상태 표시로 축약.
...
> 근거: 계획서 §14 F53(단일 출처 — 계층 분리 정체성·②-1 저장 방식 3안 비교·②-2 화이트리스트·②-3 토큰 값 분리/size 명명 통일·③ 우선순위·④ 경계 규약·⑤ 폰트·R27). 구현 중 이 계약과 어긋나는 필요(특히 신규 엔드포인트)가 발견되면 임의 확정 없이 착수 중단 후 보고한다(stage-28 범위 요약).
> 원칙 재확인: **두 계층은 저장 위치·적용 범위·우선순위가 전부 다른 별개 계층**(합치는 설계는 착수 전 기각) · 색상은 토큰만(불변 규칙 5 — 전역 테마는 그 규칙의 배당금) · 임의 JSON 수용 금지(화이트리스트 검증 — 규칙 5 재관통 방지) · 에러 규약 §3 · 우선순위 = **문서 지정값 > 전역 설정 > 기본 토큰**(문서 미지정 항목은 전역 상속 — 부분 지정 허용).

**① 문서별 스타일 — documents PATCH `style` 필드 계약 (신규 엔드포인트 0 — §4.2 PATCH 확장. DDL 1건: `documents.style` TEXT NULL, §6.2 정본·Alembic 세트)**

- 요청: `PATCH /api/documents/{id}` body에 `style?: {font?, size?, bg?} | null` — **JSON 객체**(TEXT 컬럼 직렬화 저장).
- **서버 화이트리스트 검증(범위 밖 = 422)**:

  | 키 | 허용 값 | 렌더 근거 |
  |---|---|---|
  | `font` | `sans` \| `serif` \| `mono` | 시스템 폰트 스택 프리셋 3종(⑤ — 웹폰트 번들 기각·임의 글꼴명 기각) |
  | `size` | `small` \| `default` \| `large` \| `xl` | 기존 FontScale(F36-⑨) 3단계 명명 + `xl`(②-3 ⓑ 통일 — 기존 코드 명명이 이긴다). **지정 문서에서는 FontScale을 대체**(④ ⓒ — 곱연산 기각) |
  | `bg` | `red` \| `orange` \| `yellow` \| `green` \| `blue` \| `purple` \| `gray` | F52 팔레트 7색 **이름**(F52 결정 ③ 일관). 값은 문서 배경용 연한 틴트 토큰 **`--doc-bg-{색}`**(라이트·다크 쌍 — F52 형광펜 `--mark-*`와 이름 체계 공유·**값 분리**, ②-3 ⓐ) |

- **부분 지정 허용**: 있는 키만 검증·저장(예: `{"bg":"yellow"}` — font·size는 전역 상속). **`style: null` = 전체 해제**(NULL 저장 — 기본 상태). 미지 키·임의 hex(`{"bg":"#ff0000"}`)·범위 밖 값 = **422**(스타일만 무시하는 관용 없음 — 저장 계층은 F52 렌더 계층과 달리 엄격: 저장된 쓰레기는 영구 표면이 된다).
- `GET /api/documents/{id}` 응답에 `style` 포함(NULL = 미지정). `PATCH` 응답 동일.
- **적용 범위는 프론트 규약**: 그 문서 **본문 렌더 영역만**(앱 크롬 불변 — R27 ③). screens §5.3 S28.

**② 임베드 무시 = 계약 수준 봉인 (resolve-embeds에 `style` 부재)**

- `POST /api/documents/resolve-embeds`(§4.19) 응답 항목 스키마에 **`style` 필드를 추가하지 않는다** — 필터링이 아니라 **부재**(F43이 answer·explanation을 봉인한 기법과 동일). 임베드 카드 안에서 문서 스타일이 무시되는 것(④ ⓐ — 카드 스타일이 이긴다)이 렌더러 규칙이 아니라 계약 수준에서 끝난다. 이 봉인이 ②-1 3안 비교에서 안 1(컬럼)이 안 2(본문 directive)를 이긴 결정 축이다.

**③ 전역 앱 테마 — settings 키 `ui.theme_custom` (기존 settings API 재사용 — 신규 엔드포인트 0·DDL 0)**

- 형태: `ui.theme_custom` **키 1개** — `{light?: {…}, dark?: {…}}`. **라이트·다크 각각 별도 저장**, 미설정 테마·미설정 항목은 **기본 토큰 상속**(한쪽만 지정해도 테마 전환이 깨지지 않는다 — ④ ⓓ). 각 테마 객체의 항목 = 폰트(프리셋 3종)·기준 글자크기·배경/서피스 계열·글자/강조색 계열 토큰 오버라이드(구체 토큰 키 집합은 구현 재량 — 아래 자유도 경계만 계약).
- **자유도 경계(① 확정 — 문서별과 다르게 가는 지점)**: **배경·서피스 계열 = 자유 색 허용 + 명도 대비 자동 검증**(글자 토큰과의 대비 미달 조합 = **저장 거부 422** — R27 ②. 전역은 반입 경로와 무관(설정 저장소 — content 밖)하고 인쇄에도 안 나가므로(인쇄 라이트 강제) 자유 선택의 부작용 표면이 좁다. 대비 임계값 구체 수치는 구현 실측 — stage-28 완료 기록) · **글자색·강조색 계열 = 팔레트/프리셋 값만**(자유 hex = 422 — 본문 위 가독성 직결, 배경보다 실패 비용이 크다).
- 검증 위치: 기존 settings PUT의 **키별 검증 훅**(이 키에 한해 — 다른 settings 키 계약(§4.10) 불변). 서버 완성 문장으로 거부 사유 반환(§3).
- 적용 = **`<html>` 수준 CSS 변수 런타임 주입**(빌드 불요 — screens §7). 프리셋 몇 종(세피아·고대비 등) + 커스텀(목록 구현 재량 — 프리셋도 저장 형태는 같은 키의 값).
- **복구 경로(R27 ① — 계약)**: 설정 화면 [기본값으로 되돌리기] 상시 노출(키 삭제/초기화) + 토큰 무시 안전 모드 1개(URL 쿼리 등 — 구체 형태 구현 재량. 커스텀이 아무리 깨져도 기본 토큰으로 진입 가능한 문).

**④ 경계 규약 전수 (확정 2026-08-09 — screens §5.3·§6 연동)**

| 경계 | 계약 |
|---|---|
| 임베드 카드 안 | 문서 스타일 **무시**(위 ② — 응답 부재로 봉인. 상위 문서 화면 누더기 방지) |
| 인쇄(F22) | **배경 전부 무시**(전역·문서 공통 — §6 "인쇄는 항상 라이트" 연장. `--doc-bg-*`에는 `print-color-adjust: exact`를 걸지 않는다 — F52 형광펜·글자색 한정 결정 불변). **폰트·글자크기는 인쇄 유지**(잉크와 무관 — 정리본이 화면과 같은 서체) |
| FontScale(F36-⑨) | 문서 `style.size` 지정 시 **대체**(size 미지정 문서는 기존 동작 그대로 — 중복 축 방치 없음). F52 `:t` `s=` 상대 배율(em) 곱 합성은 별개 계층 — 무변경 |
| 다크 모드 | 커스텀은 라이트·다크 **각각 저장** — 테마 전환 상시 동작(한쪽만 지정 = 다른 쪽 기본 토큰) |
| 반입 경로 | §8.2 반입 JSON 규격 무변경 — LLM·URL·문서 추출이 style을 만들 경로 0(②-1 "반입 경로 격리") |

**DDL 1건·신규 엔드포인트 0개 — 재확인 근거 (2026-08-13, 착수 문서화)**

- 이 절의 저장 지점 전수: **`documents.style` 컬럼**(DDL 1건 — nullable·기본 NULL·기존 행 소급 0, §6.2 갱신 완료(2026-08-13) + Alembic 세트는 stage-28 구현분) · **settings 키 1개**(`ui.theme_custom` — 기존 테이블·기존 API). **새 엔드포인트 0**(documents PATCH 필드 확장 + settings PUT 재사용). LLM 0. 구현 중 이 전제가 깨지면 착수 중단 후 보고(임의 확정 금지).

**구현 앵커 (2026-08-13 — 파일 수준. 행 번호는 구현 시 실측)**

- 백엔드: `backend/models.py`(`style` 컬럼) + `backend/alembic/`(신규 리비전 — 왕복 검증) · documents 라우터/스키마(`style` 화이트리스트 검증·응답 포함) · settings 저장 경로(`ui.theme_custom` 키별 검증 훅) · resolve-embeds 스키마(무변경이 정상 — 회귀 확인만).
- 프론트: 설정 화면 그룹 ⑥(전역 테마 편집 — screens §5.11) · `styles/tokens.css`(`--doc-bg-*` 14개) · 테마 주입 유틸(신규 — `<html>` CSS 변수·screens §7) · DocEditor/문서 상세(문서 스타일 폼 — screens §5.3) · 본문 렌더 래퍼(우선순위 적용·임베드/인쇄 경계).

### 4.27 이미지 첨부 — 업로드·붙여넣기·드래그앤드롭 (S29 — F54. **계약 확정 2026-08-09, 착수 문서화 2026-08-14 — stage-29 착수 전 결정 ①~④ 전건 해소분**)

> 근거: 계획서 §14 F54(단일 출처 — ① 업로드 엔드포인트 1개·② MIME 화이트리스트(svg 제외)·③ 10MB·재인코딩 없음·④ 편집 UX·범위 밖 확정·스키마 영향). 구현 중 이 계약과 어긋나는 필요(특히 DDL·신규 의존·엔드포인트 증가)가 발견되면 임의 확정 없이 착수 중단 후 보고한다(stage-29 범위 요약).
> 원칙 재확인: **신규는 업로드 1개뿐** — 저장 디렉터리·서빙 라우트·경로 탈출 차단·내용 해시 중복 제거·백업 포함은 **기존 자산 재사용**(2026-08-09·2026-08-14 코드 실측). **판별은 내용(매직 바이트)이 정본** — 파일명·`content-type`은 판정에 쓰지 않는다(§4.18 ②-3 관례·R14 교훈). 에러 규약 §3(코드 4종 불증) · 불변 규칙 4(쓰기만·수정/삭제 코드 0) · 불변 규칙 5(프론트 신규 색 0).

**① 요청 — `POST /api/uploads`(경로명 구현 재량, 이 표기가 계약 기준)**

| 항목 | 계약 |
|---|---|
| Content-Type | `multipart/form-data`만. 그 외 = **422**(기존 `convert`·`split` 라우터의 "지원하지 않는 요청 형식입니다" 관례 문장 재사용) |
| 필드 | **`file`** 1개(기존 반입 경로 `convert`·`imports`와 같은 필드명 — 이름 통일). 파일 필드 부재·빈 파일 = 422(`reason='no_file'`) |
| 다중 파일 | **요청당 1개**(여러 장은 클라이언트가 **순차 호출** — 서버 배열 수용 없음. 부분 실패 표현·순서 보장이 클라이언트 루프로 자연 해결된다) |
| 인증 | 없음(홈 네트워크 전용 — R12. 기존 전 엔드포인트와 동일) |

**② 성공 응답 — `200 OK`**

```json
{ "url": "/images/3f2a91c7b04e5d18.png" }
```

- **필드는 `url` 하나뿐**(YAGNI — 프론트가 필요한 것은 삽입할 경로뿐이다. `bytes`·`filename`·`deduplicated` 등 추가 금지).
- **신규 저장과 중복(기존 파일 재사용)의 응답이 동일**하다 — 201/200 구분도 두지 않는다(클라이언트가 구분할 이유가 0이고, "새로 저장됐는지"는 사용자에게 의미 없는 정보다).
- `url`은 항상 **절대 경로 `/images/{16진수 16자}.{ext}`** — 기존 서빙 라우트(`main.py` `GET /images/{filename:path}`)가 그대로 서빙한다.

**③ 매직 바이트 판별 규칙 (① 확정 — 확장자·content-type 사칭 방어)**

| 포맷 | 시그니처(선두 바이트) | 결정 확장자 |
|---|---|---|
| PNG | `89 50 4E 47 0D 0A 1A 0A` | `png` |
| JPEG | `FF D8 FF` | **`jpg`**(`jpeg`로 저장하지 않는다 — 기존 `_save_fetch_images`와 동일 정규화) |
| GIF | `GIF87a` \| `GIF89a` | `gif` |
| WebP | `[0:4] == "RIFF"` **그리고** `[8:12] == "WEBP"` | `webp` |

- **구현체는 앱 전체에 1개** — 기존 `backend/services/convert_service.py`의 `_detect_image_magic(data) -> 'png'|'jpg'|'gif'|'webp'|None`을 **그대로 재사용**한다(중복 구현 금지. 공용 모듈로 이동하는 것은 구현 재량이며, 이동 시 기존 호출부·테스트를 함께 갱신).
- WebP 판정에 12바이트가 필요하므로 판별은 **선두 최소 12바이트 확보 후** 수행한다.
- **위 4종에 해당하지 않으면 전부 거부** — svg(② 확정 제외 — 스크립트를 품을 수 있어 F52 결정 ①(raw HTML 기각)의 우회로가 된다)·pdf·zip·HTML 오류 페이지·실행 파일이 모두 같은 경로로 귀결된다.
- **폴백 금지**: 판별 실패 시 확장자나 `content-type`으로 되돌아가 추측하지 않는다(`_save_fetch_images`의 `.get(ctype, "png")` 기본값 폴백은 **내가 요청한 URL의 응답**이라 신뢰 조건이 다르다 — 업로드 경로에 복제 금지). 이것이 R14 실측 교훈("`.pdf` 파일명이면 HTML 오류 페이지도 통과하던 검사")의 이행 지점이다.

**④ 확장자·파일명 결정 규칙 (사용자 제공 파일명 신뢰 0)**

- 저장 파일명 = **`{sha256(전체 바이트).hexdigest()[:16]}.{③의 판별 확장자}`** — 사용자 파일명은 **경로 계산에 일절 쓰지 않는다**(로그·클라이언트 안내 문구에만 쓸 수 있다). 결과가 서빙 정규식 `^[0-9a-f]{16}\.(gif|png|jpg|jpeg|webp)$`를 항상 충족한다(R16 이중 차단 유지 — 서빙 라우트는 **무변경**).
- 저장 위치 = `sources/images/`(기존 디렉터리 — `mkdir(parents=True, exist_ok=True)`).
- **중복 = 쓰기 생략**: 같은 경로 파일이 이미 있으면 **바이트를 다시 쓰지 않고** 같은 URL을 반환한다(내용 해시라 같은 파일명 = 같은 내용. 덮어쓰기 0 — 불변 규칙 4 정합). 부분 기록 파일이 남지 않도록 임시 파일 → rename 권장(구현 재량).
- **DB 기록 0**: `sources` 테이블 행도, 이미지 메타 테이블도 만들지 않는다(**DDL 0건** — 어느 문서가 어느 이미지를 쓰는지는 본문이 단일 출처). 백업(F27)은 `sources/` zip 스냅샷이라 **개정 0건**(코드 변경 없이 자동 포함).

**⑤ 크기 상한과 적용 지점 (③ 확정 — 파일당 10MB)**

- 상한 = **10MB(10 × 1024 × 1024 바이트) / 파일**. 초과 = 422(`reason='too_large'`) + **파일 저장 0**.
- **적용은 스트리밍 중 조기 차단** — 본문을 청크로 읽으며 누적 바이트가 상한을 넘는 즉시 중단한다(전체를 메모리·임시 파일에 받은 뒤 재는 방식 금지).
- `Content-Length` 헤더가 있으면 읽기 전 **1차 차단**에 쓰되 **신뢰하지 않는다** — 실제 누적 바이트가 정본(헤더 위조·chunked 전송 대비).
- **재인코딩·리사이즈·썸네일·EXIF 스트립 없음** — 원본 바이트를 그대로 저장한다(Pillow 등 **신규 의존 0** — F42/R19 "포맷당 파서 1개·계획서 등재 후 추가" 원칙).

**⑥ 에러 표 (§3 규약 — 코드 집합 4종 불증, 구분자는 `detail.reason`)**

| 조건 | 상태 | code | detail.reason | message 형태(한국어 사람 말 + 다음 행동) |
|---|---|---|---|---|
| multipart 아님 | 422 | `VALIDATION_ERROR` | (기존 관례 detail) | "지원하지 않는 요청 형식입니다(multipart/form-data만 허용)" |
| `file` 필드 없음·빈 파일 | 422 | `VALIDATION_ERROR` | `no_file` | "올릴 이미지 파일이 없습니다" |
| 10MB 초과 | 422 | `VALIDATION_ERROR` | `too_large` | "이미지가 너무 큽니다(10MB 이하만 올릴 수 있습니다)" |
| 매직 바이트 4종 불일치(svg·pdf·위조 확장자 등) | 422 | `VALIDATION_ERROR` | `unsupported_type` | "이미지 파일이 아닙니다. PNG·JPG·GIF·WebP만 올릴 수 있습니다" |
| 디스크 쓰기 실패 등 서버 사정 | 500 | `INTERNAL` | — | 기존 공통 핸들러 |

- **413·415는 쓰지 않는다** — §3이 코드 집합을 4종으로 고정했고(상태 코드는 각 코드의 기본값, `INTERNAL`의 500/502만 예외), 원인 구분은 이미 `detail.reason` 전례(§3 상류 실패 규약)가 담당한다. 프론트는 `code`로 분기하지 않고 **서버 `message`를 그대로 렌더**한다.
- **거부 시 파일 시스템 부작용 0**(부분 파일·임시 파일 잔존 없음).
...
**⑦ POST 확장 — 생성 시 블록 동반 [S36 — M35 표면 통합. 계약 확정 2026-08-19, 지시서 `stage-36-surface-unify.plan.md`]**

- `POST /api/documents`가 ②와 **같은 동반 규칙·얕은 검증·크기 상한**으로 `content_blocks`↔`content` · `explanation_blocks`↔`explanation` 쌍을 받는다(한쪽만 = 422 `projection_required` · 에러 표 ⑥ 그대로). 동반 생성된 문서는 **태생 전환**(`blocks_version`은 ②와 동일하게 서버가 컬럼 사본으로 채운다). 검증 로직은 PATCH 구현과 공용(복붙 금지).
- **블록 미동반 POST = 종전 계약 그대로 미전환 생성**(하위 호환 무변 — 반입·생성 서버 경로(F40/F42/F45 등)와 구 편집기는 1바이트도 변하지 않는다). **신규 엔드포인트 0 · 목록·batch·봉인(⑤) 무변.**
- 배경: M35 편집 표면 통합으로 신규 작성 경로(`/docs/new`·탐색/커리큘럼 create 모달)가 새 편집기를 쓰게 되면서, 생성 시점부터 블록 쌍을 저장할 길이 필요해졌다(생성 후 PATCH 2회 왕복은 기각 — 절반 저장 상태가 생긴다).
...
| 메서드/경로 | 설명 | 단계 |
|---|---|---|
| `POST /api/web-embed/preview` | body `{"url": "https://…"}` → 대상 페이지 메타 추출. **DB 쓰기 0 · 파일 쓰기 0 · LLM 0**(순수 조회 — 캐시 저장은 클라이언트가 블록 JSON 안에 한다) | S37 |

- 응답: `{"url", "title", "description", "site_name", "image", "favicon", "fetched_at"}` — `url` 외 전부 **문자열|null**(추출 실패 필드는 null·부분 성공 허용). `image`·`favicon`은 **원격 URL 그대로**(서버가 다운로드·저장하지 않는다 — `sources/` 무접촉·불변 규칙 4 무관).
- **캐시·오프라인 계약(클라이언트)**: 응답 메타는 **웹 임베드 블록의 prop으로 저장**되고(블록 JSON = 소스 — D1), 렌더는 항상 저장분을 쓴다(재조회는 사용자의 [메타 새로고침] 명시 조작뿐 — 열람·편집 진입 시 자동 재fetch 금지). 오프라인·썸네일 로드 실패 시 = 저장된 제목·도메인 텍스트 카드로 강등(썸네일 자리는 파비콘/도메인 이니셜 폴백 — 조용한 실패 없음·기능 성립).
- 메타 조회 실패는 **삽입을 막지 않는다** — URL만의 카드로 삽입(제목 없는 카드 = 정상 상태).

**② SSRF·자원 가드 (전건 서버 강제 — 계약)**

| 가드 | 계약 |
|---|---|
| 스킴 | `http`·`https`만(그 외 = 422 `invalid_url`) |
| 주소 차단 | **호스트명을 해석한 결과 IP**가 루프백·사설 대역(RFC1918)·링크로컬(169.254/16 — 클라우드 메타데이터 포함)·멀티캐스트이면 422 `blocked_address`. **리다이렉트 추적 시 매 홉 재검증**(첫 홉만 검사하는 우회 금지) · 리다이렉트 최대 5회 |
| 응답 제한 | `Content-Type`이 HTML 계열일 때만 본문 파싱 · 본문 읽기 상한 **512KB**(스트리밍 중 조기 차단 — §4.27 ⑤ 관례) · 타임아웃 **5초** |
| 산출 제한 | 추출 필드 각 **1,000자 절단** · HTML은 파싱 후 폐기(저장·전달 0 — 원시 HTML이 클라이언트로 가지 않는다) |
| 실패 | 타임아웃·HTTP ≥400·비HTML·상한 초과 = 422 `fetch_failed`(§3 코드 4종 불증 — 전부 `VALIDATION_ERROR`·message는 한국어 사람 말). 서버 결함만 500 |

**③ iframe 화이트리스트 (프론트 계약 — 서버 무관)**

- iframe 실임베드는 **프론트 상수 도메인 화이트리스트**(YouTube 등 소수 — 목록은 상수 파일 1곳, settings 키 0·확장은 코드 수정)에 든 URL만. 그 외 전부 북마크 카드. iframe은 `sandbox` 필수·최소 허용 속성(세부는 stage-37 확정). **인쇄·프로젝션(리더) 렌더에서 iframe 금지** — 카드/링크로 강등.
