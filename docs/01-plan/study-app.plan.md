...
> 상태: **확정 준비 (Draft v0.8)** — v0.7 대비: **v1 완성(M1~M7 종료)** 후 v1.x 로드맵 확정 — M8(LLM 인프라: F34+F35-1) → M9(일상 다듬기: F36+검색·복원) → M10(콘텐츠·동기: F35-2+F26) → M11(모의고사: F25+F16). 시험 일정 무관 순차 진행(사용자 확정), 첫 착수 M8. 스키마 변경 없음
...
| **M9. 일상 다듬기** | F36 학습 UX 10건 + 검색 한국어 recall(trigram) + 복원 후 재시작 UX | 앱 열기→공부 시작이 원탭, 풀이 루프에 마찰 없음 |
