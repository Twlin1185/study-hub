...
> 상태: **확정 준비 (Draft v0.10)** — v0.9 대비: **M9 계획·설계 확정** — F37 스키마 변경 불필요 근거 확정(단계 완료·재개는 파생값 — §14 F37 명세), 검색 한국어 recall trigram 재구축 노트(§6.2 구현 노트), F36 건수 표기 정리(11건), Stage 9 작업 지시서(`stage-9-daily-polish.plan.md`)·설계 §4.12 연결. (v0.9: v1.x 기능 명세 확장 — F34 잡 진행 가시화, F36 ⑪ 공용 Stepper, F37 챕터 파이프라인, F38 설정 재구성·태그 관리자.) **스키마(§6.2 DDL) 변경 없음** — FTS 인덱스 재구축 마이그레이션만
...
| **M9. 일상 다듬기** | F36 학습 UX 11건 + **F37 챕터 파이프라인(개념→문제→기출)** + **F38 설정 재구성·태그 관리자** + 검색 한국어 recall(trigram) + 복원 후 재시작 UX — 작업 지시서 `stage-9-daily-polish.plan.md` · 설계 §4.12 | 앱 열기→공부 시작이 원탭, 챕터가 한 흐름으로 완주, 태그가 통제됨 |
...
> 구현 순서는 `docs/01-plan/stage-1-skeleton.plan.md` ~ `stage-9-daily-polish.plan.md` 참조. MVP = Stage 1~3, v1 완성 = Stage 7, v1.x = Stage 8~.
