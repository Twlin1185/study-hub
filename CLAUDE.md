...
| `docs/01-plan/stage-{1..17}-*.plan.md` | 단계별 작업 지시서 — 체크리스트·DoD·제외 범위. **stage 1~16 완료** · **17**(문서 상호참조·표현 UX F43) 착수 가능(게이트 없음) · **18**(해설·정답 보완 F44)·**19**(응용 모의고사 F45, M19)·**20**(반입 자기 개선 루프 F46, M20)은 계획서 등재만(17↔18 순서 자유, 19는 F44 선행 권장, 20은 순서 자유). 각 단계의 완료 경위·게이트 기록은 해당 stage 문서 참조 |
