...
| 문서 | 역할 |
|---|---|
| `docs/01-plan/study-app.plan.md` | 마스터 계획 — 기능 F01~F44, **스키마 DDL 단일 출처(§6.2)**, 로드맵 M1~M18, 리스크 R1~R21 |
| `docs/02-design/study-app.design.md` | API 명세(엔드포인트에 [S1]~[S14] 태그) + 화면 상세 12개 + 토큰/상태관리 |
| `docs/01-plan/stage-{1..17}-*.plan.md` | 단계별 작업 지시서 — 체크리스트·DoD·제외 범위. MVP = stage 1~3, v1 = ~7, v1.x = 8~11, 후속 = 12~14 (**stage 1~14 완료** — 2026-07-28: 13 = 사설 어댑터 제거 + 수동 반입 UX, 14 = 큐넷 공식 오픈API). **stage 15**(멀티 벤더 LLM 엔진 — Codex CLI)는 착수 게이트 3건 통과(G2 품질 재실험 = `Desktop\codex_engine_g2\results\G2_report.md`) 후 **구현 완료**(2026-07-28 — 엔진 레지스트리·codex 어댑터(stdin 전달)·변환 신뢰 게이트·S8 온보딩). **stage 16**(다양한 문서 포맷 반입 F42 — 판별·거부 계층 + A군 직투입 정식화 + docx/xlsx 서버 추출(python-docx·openpyxl) + `too_large`/`parse_failed` 구조화)은 **구현 완료**(2026-07-29 — 설계 §4.18, Opus 재검토 통과). **stage 17**(문서 상호참조·표현 UX F43)은 계획 등록(게이트 없음). **stage 18**(해설·정답 보완 플로우 F44 — 답지 반입 정본 + LLM 풀이 보조)은 계획서 등재만(2026-07-29 — stage 문서는 착수 시 생성, 17↔18 순서 자유) |
