...
> 상태: **확정 준비 (Draft v0.60)** — v0.59 대비: **stage-52 통합 휴지통 편성 + 불변 규칙 4 개정 (2026-09-14 — 문서만·코드 0. 스키마(§6.2) 무변경 · §5 F61 행 + §14 M38 행 + §15 R43 행 추가 · §6.3 4 개정)**: 별지 §10 D8 확정(2026-08-23 — 고아 이미지 2단계 정리 · 삭제 주체 = 항상 사용자 · 앱 자동 삭제 0)의 구현 편성 → **`stage-52-trash-orphan-images.plan.md`**(문서·노트 소프트 삭제분 목록 + 복원 · 고아 이미지 스캔 → `sources/images/.trash/` 이동·되돌리기 · 신규 엔드포인트 7 · DDL 0 · 핵심 → v2.03.1 예정). **§6.3 불변 규칙 4 개정 이행**: `sources/images/` 업로드·수집 이미지 = 앱 생성 파생물(고아 식별·휴지통 이동만 · 자동 삭제 0 · 최종 삭제 = 사용자 수동) · 반입 원본은 종전대로 불변 — `CLAUDE.md` 규칙 4 줄 동기. R43 = 파일 이동 코드 도입 기록(경로 가드·참조 재검사·삭제 0 봉인). FB-24 ⓒ는 동승하지 않음(별도 편성). 정본 = stage-52 지시서 §2 · 설계 §4.32·§5.17(Design v1.65) — 마스터에 옮겨 적지 않는다.
> (v0.59: v0.58 대비: **stage-41 2차(고정 열 다단) — 결정 ① 사용자 번복(흐름형 ⓐ → 고정 열 ⓑ) · 재구현·검토·실측 완료 문서화 (2026-08-30 — 문서만·코드 무접촉. 스키마(§6.2)·엔드포인트·기능·리스크 번호 무변경·§14 무접촉)**: 1차 흐름형 실사용에서 "단이 안 생김·3단 입력이 1단으로 이동"(브라우저 다단 균등 배분) → 사용자 확정 ⓑ → `columns` > `column` 컨테이너 grid + 방언 `::::columns{n=2}`/`:::column` + 리더 grid 셀 · Opus 검토 조건부(치명 0·중요 3·경미 7) → 수정(단 경계 키 가드 강등 · 정규화 두 계층 동치 · 헤드리스 72건) · 브라우저 실측 전건 · 실측 = **백엔드 diff 0·신규 의존 0·초기 청크 Δ vs main JS +1,232B/CSS +63B**·회귀 무변·s41 335/335 · **잔여 = DoD 8(사용자 PC+폰 ⓐ~ⓕ) + 머지** · 설계 **Design v1.51**(screens §5.16 S41 2차). 상세 정본 = `stage-41-flow-columns.plan.md` 완료 기록(2차) — 마스터에 옮겨 적지 않는다.)
...
> (v0.56 이하 문서 이력 원문(v0.15~v0.56 · 2026-07-31 구조 변경 기록 포함)은 `docs/04-archive/plan-changelog.md`로 이관 — v0.55 이하 = 2026-08-31 stage-44 · v0.56 = 2026-09-14 stage-52 편성(v0.60) 시 밀려남. 위에는 현행 상태 줄 + 최근 3건(v0.59~v0.57)만 잔류.)
...
| F60 | **탐색 문서 일괄 도구 — 다중 선택(체크박스·Shift 범위·현재 목록 전체 선택) + 선택 툴바(분류에 연결·분류 이동·연결 해제·소프트 삭제) · `POST /api/documents/bulk`(§4.31 · 한 트랜잭션 · 삭제 = `is_active=0`만) · 문서 상세 [이동] 1동작**(stage-51 편성 시 부여, 2026-09-13 — 색인 행. 상세 정본 = `stage-51-explore-bulk-select.plan.md` · 출처 = 별지 §13 FB-25) | M37 |
| F61 | **통합 휴지통 — 문서·노트 소프트 삭제분 목록 + 복원(`POST /api/documents/{id}/restore`·`/notes/{id}/restore` · `GET /api/trash/documents`·`/notes`) + 고아 이미지 2단계 정리(`GET /api/trash/images` 스캔 → `POST /api/trash/images/move`·`/restore` — `sources/images/.trash/` 이동만 · **앱 자동 삭제 0 · 최종 삭제 = 사용자 수동**) · 설계 §4.32 · 화면 §5.17(`/trash` · 설정 데이터 카드 진입) · DDL 0 · 불변 규칙 4 개정 동반(§6.3)**(stage-52 편성 시 부여, 2026-09-14 — 색인 행. 상세 정본 = `stage-52-trash-orphan-images.plan.md` · 출처 = 별지 §10 D8) | M38 |
...
4. **원본 불변** — sources/의 **반입 원본 파일**은 수정·삭제하지 않는다. 정리본(문서)만 다듬는다. **(stage-52 개정 2026-09-14 · 별지 D8 확정 2026-08-23 이행)** `sources/images/`의 업로드·수집 이미지는 **앱 생성 파생물**로 재해석한다 — 앱은 미참조(고아) 식별과 `sources/images/.trash/`로의 이동·되돌리기까지만 하고(코드 = `services/trash_service.py` 1곳 · 경로 가드 R16) **자동 삭제 0 · 최종 삭제 = 사용자 수동**(탐색기에서 폴더 비움). 반입 원본(`sources/images/` 밖)은 종전대로 불변 · 삭제 함수 호출 0.
...
> 완료 마일스톤의 원문 행·결정 기록·말미 경과 서사는 `docs/04-archive/roadmap-history.md`(M1~M16 = 2026-07-31 이동 · M17~M36 = 2026-08-31 stage-44 이관), 완료 기능 상세 명세(F16·F25·F26·F34~F56 — F35-2 제거 이력·F35-3 실측 확정 포함)는 `docs/04-archive/feature-specs-done.md`에 **원문 그대로** 보존돼 있다. §14에는 압축 표만 잔류(2026-08-31 기준 M1~M36 전 행 완료 · **M37 = 2026-09-13 구현·검토 완료 · v2.02.1 발행 대기**). 각 단계의 작업 지시서·완료 경위는 `stage-{n}-*.plan.md`가 단일 출처.
...
| **M38. 통합 휴지통(F61 — 별지 D8-구현) → v2.03.1 예정** | 문서·노트 소프트 삭제분 목록 + 복원 · 고아 이미지 식별 → `sources/images/.trash/` 이동·되돌리기(앱 자동 삭제 0 · 최종 삭제 = 사용자 수동) · 불변 규칙 4 개정(§6.3 — 업로드·수집 이미지 = 파생물 · 반입 원본 불변 유지) · 신규 엔드포인트 7 · DDL 0 — 지시서 `stage-52-trash-orphan-images.plan.md` | ✅ 2026-09-15 구현·검토 완료 — **v2.03.1 발행 대기**(V-3 실측 + DoD 6 회신 후 발행 · 핵심 · FB-24 ⓒ는 이 뒤 별도) |
...
| R43 | **`sources/images/` 파일 이동 코드 도입(M38/stage-52) — 불변 규칙 4 개정 기록**(2026-09-14 편성 · 스키마 아님 · 규칙 개정이라 등재). 사유: 별지 D8 확정(고아 이미지 2단계 정리 · 삭제 주체 = 사용자)을 이행하려면 앱이 처음으로 `sources/` 아래 파일을 **옮기는** 코드를 가진다. 위험 3가지 — ① 경로 가드 결함 시 `sources/images/` 밖 파일이 이동될 수 있다(R16 계열) ② 참조 판정 결함 시 사용 중 이미지가 휴지통으로 가 본문 깨짐(비활성 문서·반입 미리보기·편집 중 미저장분이 오판 지점) ③ "이동만"이 시간이 지나며 "삭제"로 확장될 유혹(원칙 침식) | ① **파일명 = 서빙 정규식 fullmatch + `resolve()`·`is_relative_to` 이중 검사 · 방향 고정(`images/` ↔ `images/.trash/`) · 이동 코드는 `trash_service.py` 1파일** · invariant `fs-mutate` 기준선은 그 2곳만 정당(다른 파일 신규 검출 = 결함) ② 참조원 = documents 5컬럼 + notes 2컬럼 **활성+비활성 전 행** + `import/auto/*.json` · **이동 직전 재검사**(참조 중 = 422 all-or-nothing) · 7일 mtime 유예 · 되돌리기 제공 · 백업 zip에 `.trash/` 자동 포함 ③ **삭제 엔드포인트·자동 비우기·보존 기간 트리거 0을 지시서 §4 "하지 않는 것"으로 봉인** — 확장 요구는 별지 D 재론 절차(사용자 확정) 없이는 착수 금지. 감시: 새 이미지 참조 지점(컬럼·파일)이 생기면 참조원 목록 동반 갱신 |
