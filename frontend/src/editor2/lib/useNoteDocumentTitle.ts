// 노트(베타) 화면의 **브라우저 탭 제목**을 앱 나머지와 구분되게 바꾼다.
//
// 왜 필요한가: `index.html`의 `<title>Study Hub</title>` 하나뿐이라 앱 전체가 어느 화면이든
// 같은 탭 제목으로 뜬다. 개발 중에는 기존 문서 화면과 노트(베타)를 나란히 열어 두는데, 탭·작업
...
/**
 * 노트 화면이 떠 있는 동안 탭 제목을 `노트(베타) …`로 바꾼다.
 *
...
export function useNoteDocumentTitle(suffix?: string): void {
  useEffect(() => {
    ...
    document.title = trimmed ? `노트(베타) · ${trimmed} — ${BASE_TITLE}` : `노트(베타) — ${BASE_TITLE}`
    return () => {
