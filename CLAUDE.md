...
| 문서 | 역할 |
|---|---|
| `docs/01-plan/study-app.plan.md` | 마스터 계획 — 기능 F01~F48, **스키마 DDL 단일 출처(§6.2)**, 로드맵 M1~M22(압축 표 + 진행·예정 F43~F48 상세), 리스크 R1~R24 |
| `docs/02-design/study-app.design.md` | 설계 **색인** — §1~3(범위·구조·API 공통 규약) + 파일 분할 지도. § 번호는 분할 전과 동일 |
| `docs/02-design/study-app.design.api.md` | §4 API 명세(4.1~4.24, [S1]~[S22] 태그) — 필요한 절만 `### 4.<n>` Grep 후 부분 읽기 |
