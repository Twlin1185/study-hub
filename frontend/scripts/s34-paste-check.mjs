...
// 느슨한 목록 전처리(U-3) — 붙여넣기 경로가 이 함수를 거친다(F절).
...
console.log('== F. 느슨한 목록 HTML 표식 보존(U-3) ==')
{
  ...
  check(
    'U-3: 전제 재현 — 전처리 없이는 `<li><p>` 목록이 tight로 접힌다',
  ...
  check('U-3: tight 목록 HTML은 전처리가 문자열을 그대로 돌려준다', expandLooseLists(tightHtml) === tightHtml)
  check(
    'U-3: tight 목록은 여전히 tight다(빈 줄을 만들지 않는다)',
  ...
  check('U-3: BlockNote 자체 복사 HTML은 느슨함의 근거가 아니다', expandLooseLists(bnCopyHtml) === bnCopyHtml)
  check(
    'U-3: 그래서 편집기 내부 복사·붙여넣기에 없던 빈 줄이 생기지 않는다',
  ...
  check('U-3: 항목이 1개인 목록은 전처리 대상이 아니다', expandLooseLists(singleHtml) === singleHtml)
  // 목록이 없는 붙여넣기는 파싱조차 하지 않는다(경로 무접촉).
  check('U-3: 목록이 없는 HTML은 원본 문자열 그대로다', expandLooseLists('<p>문단</p>') === '<p>문단</p>')
  // D절의 "목록 2개" 표본(이미 살아남던 경로)도 전처리가 건드리지 않는다.
  const twoLists = '<ul><li>A</li><li>B</li></ul><ul><li>C</li><li>D</li></ul>'
  check('U-3: 이미 살아남던 "목록 2개" 표본은 무접촉이다', expandLooseLists(twoLists) === twoLists)
