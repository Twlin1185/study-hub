...
- **진입·구성**: 모의고사 구성 화면(위)에 모드 전환 탭 **[실전(기출)] / [AI 응용]** 추가. AI 응용 탭 = ① 범위 선택(기존 분류 트리 선택 재사용 — 실전 트리에서 고른다) → ② 문항 수(1~20)·엔진 선택(**S21, F47 — §4.23 ⓒ**: 공용 EngineSelect 게이팅 — `available:false`·`enabled:false` 엔진은 선택 불가 + 사유 1줄("설치 필요"/"로그인 필요"/"키 미등록"/"사용 안 함") + [다시 확인] 재진단 연계. 명시 지정 비가용·비활성 엔진은 서버도 422로 거부 — 프론트만 믿지 않는다) → [생성 준비] = `POST /api/applied-exam/prepare`(LLM 0).
