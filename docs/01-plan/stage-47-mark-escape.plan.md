...
> 상태: **착수 전(편성 2026-09-12).** 착수 게이트 1건 = §2 규약 A-③(스페이스 2회의 결과 형태 — 기본값
> 채택안에 사용자 한 줄 회신). 그 외 규약은 위임 판정으로 확정.
...
- **A. 마크 탈출 제스처(FB-20) — 공통 정의**
  - **A-① 대상 마크 = 스타일 마크 전부**(`editor.schema.styleSchema` 등재 키 — bold·italic·underline·strike·
    code·spoiler·highlight·`t`(글자색·바탕색·크기) 등). 스포일러·형광펜만 특별 취급하지 않는다. 근거:
  ...
  - **A-③ 제스처 3종(전부 구간 끝에서만 발동 · 그 밖에는 `false` 반환 = 코어·브라우저 기본 동작 그대로)**:
    - `ArrowRight`: **커서 이동 없이** `pending` 스타일 마크를 대기 마크에서 제거(`tr.setStoredMarks(현재
      ...
      (사용자 제안은 "빠져나오기" — 공백은 스페이스 제스처가 담당).
    - `Tab`: 구간 끝이면 위와 동일 처리 → `true`(코어 `nestBlock` 들여쓰기 **미실행**). 구간 끝이 아니면
    ...
    - `Space` **2회**: 1회째는 평소대로(코어가 상속 마크 붙은 공백 삽입). 2회째 = "구간 끝 + **직전 문자가
      ...
      정상 삽입된다(폭주 없음). **← 사용자 확정 대기(착수 게이트)**: 대안 ⓑ = 두 번째 공백도 삽입(마크
      밖 공백 2개). **기본값 = 위 ⓐ(치환·공백 1개)** — 근거: 문장 안 이중 공백 잔존 0 · 모바일(Tab·→
      없음)의 주 제스처라 결과가 단순해야 함 · 코어 link exit도 공백 1개 관례. 이의 없으면 ⓐ로 착수.
...
- [ ] V-3. **브라우저 실측**(사용자가 띄운 `localhost:8000` 우선 — 불가 시 임시 포트 자체 기동 허용 · **8000
...
- [ ] D-1. **screens §5.16 S47 블록 실측 재개정 = Design v1.56**(편성 추기 v1.55 → 구현 실측 반영 · 색인 헤더
...
- [ ] D-2. `editor-v2.plan.md` §13 — FB-20 행·FB-6 행(후속 ① 문구 뒤)·475행 고아 모듈 불릿에 `← 완료(stage-47 ·
      날짜)` 1줄씩 추기(관례).
- [ ] D-3. 매뉴얼 `docs/manual/user-manual.html` `#doc-inline-format` 절 단축키 문단(557~558행 부근)에 **마크
