...
| `docs/02-design/study-app.design.api.md` | §4 API 명세(4.1~4.22, [S1]~[S20] 태그) — 필요한 절만 `### 4.<n>` Grep 후 부분 읽기 |
...
| `docs/01-plan/stage-{1..20}-*.plan.md` | 단계별 작업 지시서 — 체크리스트·DoD·제외 범위. **stage 1~19 완료**(17~19 잔여 사용자 확인 항목은 2026-08-02 사용자 확인으로 완료 처리 — 표본 수치 미기록) · **20**(반입 자기 개선 루프 F46, M20) 구현 완료·검토 통과(2026-08-02 — DoD 자동 검증 6/6·치명 0·중요 1(dist)은 해소, 경미 7건 보고만·라이브 완주(DoD 7)·표본 점검(DoD 8, R23)은 사용자 이행 잔여). 각 단계의 완료 경위·게이트 기록은 해당 stage 문서 참조 |
