...
| `docs/02-design/study-app.design.api.md` | §4 API 명세(4.1~4.20, [S1]~[S18] 태그) — 필요한 절만 `### 4.<n>` Grep 후 부분 읽기 |
...
| `docs/01-plan/stage-{1..18}-*.plan.md` | 단계별 작업 지시서 — 체크리스트·DoD·제외 범위. **stage 1~18 완료**(17·18은 2026-08-02 검토 통과 — 18은 라이브 완주·표본 검증(R21) 사용자 확인 3건 잔여, stage-18 문서 완료 기록 참조) · **19**(응용 모의고사 F45, M19)·**20**(반입 자기 개선 루프 F46, M20)은 계획서 등재만(19 착수 가능 — F44 표기 관례 상속 조건 충족, 20은 순서 자유). 각 단계의 완료 경위·게이트 기록은 해당 stage 문서 참조 |
