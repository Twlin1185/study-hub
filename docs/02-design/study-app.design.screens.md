...
## 5. 화면 상세 (13개 + 전역 패널 1개 + 반입 서브플로 위저드 1개 — S20에서 §5.13 제안함 절 신설: 화면 자체는 S6부터 존재·문서화는 F46 수신함 2탭 일반화 시점에 추가 · **S22에서 §5.14 LLM 작업 센터 신설: 라우트가 아닌 전역 패널** · **S23에서 §5.15 분할 반입 위저드 신설: 상시 라우트가 아닌 §5.9 `too_large` 대안 서브플로**)
