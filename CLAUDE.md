...
| `docs/01-plan/stage-{1..17}-*.plan.md` | 단계별 작업 지시서 — 체크리스트·DoD·제외 범위. MVP = stage 1~3, v1 = ~7, v1.x = 8~11, 후속 = 12~14 (**stage 1~14 완료** — 2026-07-28: 13 = 사설 어댑터 제거 + 수동 반입 UX, 14 = 큐넷 공식 오픈API). **stage 15**(멀티 벤더 LLM 엔진 — Codex CLI)는 계획 등록·**착수 게이트 있음**(M13·M14 종료 + 품질 재실험 — 문서 상단 게이트 참조). **stage 16**(다양한 문서 포맷 반입 F42)·**stage 17**(문서 상호참조·표현 UX F43)은 계획 등록(게이트 없음 — 권장 순서 16→17, M15와 16 동시 착수 금지) |
