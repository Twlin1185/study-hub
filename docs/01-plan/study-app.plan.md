...
> 상태: **확정 준비 (Draft v0.12)** — v0.11 대비: **M11 계획·설계 확정** — F25 실전 모의고사 상세 명세 신설(§14 — **전용 일괄 제출 채점**(`exam/submit`, 배치 전체 한 트랜잭션), 리포트·이력은 **attempts 파생**(런 키 = 배치 공통 answered_at) — **스키마 변경 불필요 근거 확정**, 합격선은 실행 시 파라미터(기본 40/60 국가기술자격 표준), 타이머는 프론트 주도), F16 D-Day 복습 강도 조절 상세 확정(§14 — **큐 구성 계층** 3종: 유효 상한 확대·임박 범위 우선·선행 복습, sm2 알고리즘·저장값 불변), §9 화면 12개(모의고사 추가), 리스크 R15 추가, Stage 11 작업 지시서(`stage-11-exam-tools.plan.md`)·설계 §4.14 연결. (v0.11: M10 계획·설계 확정.) **스키마(§6.2 DDL) 변경 없음 — Alembic 마이그레이션 0건**(attempts.mode 주석에 'study'·'exam' 추가 — 값 제약 아님, 주석 정정)
...
| **M10. 콘텐츠·동기** | F35 2단계(사이트 어댑터 — comcbt·큐넷 2종, 큐넷 우선) + F26 학습 목표·스트릭(F36-③ 연계) — 작업 지시서 `stage-10-content-motivation.plan.md` · 설계 §4.13 | 자격증 선택→회차 자동 반입, 스트릭이 히트맵·홈과 연동 |
| **M11. 시험 직전 도구** | F25 실전 모의고사(일괄 제출 채점·과목별 리포트) + F16 D-Day 복습 강도 조절(SRS 큐 부스트) — 작업 지시서 `stage-11-exam-tools.plan.md` · 설계 §4.14 | 실제 시험 형식 모의 응시·리포트 |
...
> 구현 순서는 `docs/01-plan/stage-1-skeleton.plan.md` ~ `stage-11-exam-tools.plan.md` 참조. MVP = Stage 1~3, v1 완성 = Stage 7, v1.x = Stage 8~.
