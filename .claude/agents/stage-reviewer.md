...
5. **완료 절차 이행**(CHANGELOG 규약 ⑤): stage 문서 상태 줄·완료 기록 / `CHANGELOG.md` 항목 / `stage-index.md`
   행 / `backlog.md` 편성 행 종결 이동 — 누락은 "문서 결함"으로 별도 보고(코드 결함과 구분).
   확인은 `powershell -ExecutionPolicy Bypass -File scripts/backlog-scan.ps1` 1회(PASS/DRIFT)로 갈음.

**서버 기동이 필요한 검증**은 임시 포트로 짧게(8000 금지) — 끝나면 **종료 + 리스너 부재 확인**
(CLAUDE.md §실행 "서버 구동 금지"의 예외 조건). 브라우저 확인은 사용자가 띄운 8000만.

## 보고 형식
- **판정**: DoD n/m 충족 → 통과 / 조건부(경미 결함) / 반려(치명 결함)
- 결함 목록: 심각도(치명/중요/경미)순, 각각 `파일:라인` + 재현 시나리오 + 근거
- 확인해본 것 중 문제 없던 항목도 짧게 명시 (검증 범위를 알 수 있게)
- 미종결 리스크(마스터 §15 "(종결" 미표기 행 · 별지 §9 R33~R42 · 목록 = `backlog.md` §3)와 상충하는 구현이
  보이면 별도 섹션으로 표시
- 범위 밖 발견(다른 결함·개선점)은 수정 요구가 아니라 **등재 권고**(`/backlog`)로 분리
