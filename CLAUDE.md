...
잔여 사용자 결정 = **N-2**(구 편집기 퇴역 **실행**) — 판정은 ⓐ 권고 확정이나 **2026-08-23 사용자 결정 = 연기**: 노트 기본 기능 구현·수정·테스트 완료 후 실행하며 **그 실행이 v2.00.1 발행**이다(그때까지 퇴역 대상 코드 무접촉).
이후 작업(캡처 파이프라인·백로그 FB-6/FB-7 등)은 **v2.x 계획서를 먼저 확정한 뒤 착수** (관례).
...
| `docs/01-plan/editor-v2.plan.md` | **문서 작성·편집 재편(v2.00.0 — 완료) 별지 계획서** — 전략·아키텍처·로드맵(M31~M36)·결정 D1~D11·리스크 R33~R41·§13 피드백 백로그·§14 프로젝션 손실 목록(정본). **현황(2026-08-22): M31~M36 전 단계 완료 = v2.00.0 발행. 사용자 대기 = N-2(퇴역 실행)·D8 고아 이미지 정리·D11-ⓐ·FB-2 [저장] 버튼. 저장소 public — GPL 의존 금지(D10) 불변** |
...
| `docs/01-plan/stage-{1..38}-*.plan.md` | 단계별 작업 지시서 — 체크리스트·DoD·제외 범위. **stage 1~38 전 단계 완료**(stage 1~30 = 2026-08-15 = v1.90.1 · stage 31~38(에디터 v2) = 2026-08-22 = v2.00.0 발행). 완료 경위·게이트 기록·잔여 항목은 **각 stage 문서**가 정본(15단계 이후는 머리말 "상태" 줄 + 말미 "완료 기록", 그 이전은 체크박스) — CLAUDE.md에 옮겨 적지 말 것 |
