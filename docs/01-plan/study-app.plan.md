...
> 상태: **확정 준비 (Draft v0.14)** — v0.13 대비: **M12 신설(콘텐츠 확장·도움말 — 실사용 요청 2026-07-26)**: ① F35 어댑터 3호 **cbtbank.kr**(사전 조사 실측 2026-07-26: robots가 `/exam/`·`/category/` 명시 Allow, 기출 열람 무로그인, 회차 페이지가 **서버 렌더 정적 HTML에 전 문항·보기·정답·해설·과목 구분·이미지 포함** — **FetchedExam 구조화 추출 경로 첫 실사용**). 중복 회차 우선순위 **큐넷 > cbtbank > comcbt**로 갱신(§14 F35 — 기존 "큐넷 정본 최우선" 결정은 유지, comcbt 아래 3순위로 확장), 회차 동일성은 **시험 날짜(YYYY-MM-DD) 자연 키** 병합 추가(cbtbank 라벨에 회차 번호 없음 — 실측). ② **F39 사용자 매뉴얼 앱 통합** 신설(`docs/manual/user-manual.html` 읽기 전용 서빙 `GET /manual` — 원본이 단일 출처). **DDL 변경 없음.** (v0.13: M8~M11 완료 표기 — v1.x 종료. v0.12: M11 계획·설계 확정 — F25·F16·R15.)
...
| F35 | 기출 소스 커넥터 — 1단계 URL 반입, 2단계 사이트 어댑터 | M8(1단계)·M10(2단계)·M12(어댑터 3호 cbtbank) |
...
| **M10. 콘텐츠·동기** | F35 2단계(사이트 어댑터 — comcbt·큐넷 2종, 큐넷 우선) + F26 학습 목표·스트릭(F36-③ 연계) — 작업 지시서 `stage-10-content-motivation.plan.md` · 설계 §4.13 | 자격증 선택→회차 자동 반입, 스트릭이 히트맵·홈과 연동 ✅ 2026-07-26 |
...
| **M12. 콘텐츠 확장·도움말** (v1.x 후속 — 실사용 요청 2026-07-26) | F35 어댑터 3호(**cbtbank.kr** — FetchedExam 구조화 추출 경로 첫 실사용, 중복 우선순위 큐넷>cbtbank>comcbt·날짜 자연 키 병합) + F39 사용자 매뉴얼 앱 통합(`GET /manual` 읽기 전용 서빙) — 작업 지시서 `stage-12-content-help.plan.md` · 설계 §4.13(갱신)·§4.15 | cbtbank 회차가 문항·정답·해설·과목·이미지까지 구조화돼 미리보기에 도달, 매뉴얼이 앱 안 진입점에서 새 탭으로 열림 |

> v1.x는 위 로드맵 M8~M11로 확정 (2026-07-24, 시험 일정 무관 순차 진행 — 사용자 확정). **M8~M11 전 단계 구현·검토·머지 완료 — v1.x 종료 (2026-07-26).** M12는 v1.x 종료 후 첫 실사용 요청(cbtbank 반입·매뉴얼 통합 — 2026-07-26)으로 신설. 아래는 각 기능의 상세 명세. F15(문서 분기)는 실수요 확인 전 보류.
...
> - **2단계 (사이트 어댑터, M10 — 설계 §4.13)**: 자격증·회차 선택 UI → 스크랩 → 문제/보기/정답/해설 구조 추출 → "정보처리기사/필기/2023년 2회" 형태 분류 자동 생성. 그림 문제는 이미지를 sources/에 저장해 본문 삽입. 어댑터는 사이트별 모듈로 분리(DOM 변경 시 해당 모듈만 수정).
>   - **대상 사이트 3곳**: **전자문제집 CBT(comcbt.com)** + **큐넷(Q-Net) 공개자료**(이상 2026-07-25 사용자 확정) + **CBT문제은행(cbtbank.kr)**(M12 — 실수요 확정 2026-07-26). 어댑터 4호 이상은 실수요 확인 후.
>   - **cbtbank 채택 근거(사전 조사 실측 2026-07-26)**: robots.txt가 `User-agent: *`에 `/exam/`·`/category/`를 **명시 Allow**(`/bbs/`·`/adm/`·`/data/` 등만 Disallow), 기출 열람에 **로그인 불필요**(그누보드 — 회차 목록·문항 페이지 공개). 회차 페이지(`/exam/bp20220424` 형태)가 **서버 렌더 정적 HTML에 전 문항·보기 4개·정답·해설·과목 구분·규칙 URL 이미지**를 포함 — comcbt(JS 앱→PDF 우회)와 달리 **FetchedExam 구조화 추출 경로를 실사용하는 첫 사이트**이고, 정답·해설이 이미 구조화돼 있어 LLM 사용량도 낮다. 라벨은 "자격증명 (YYYY-MM-DD)" 날짜형 — **회차 번호(N회) 표기가 없다**(아래 병합 규칙의 근거).
>   - **중복 회차 우선순위 = 큐넷 > cbtbank > comcbt(M12 갱신)**: 기존 "큐넷(공단 원본 = 정본) 최우선" 결정(2026-07-25 사용자 확정)은 유지하고, cbtbank를 comcbt보다 위에 둔다 — cbtbank는 문항·정답·해설·과목·이미지가 개별 구조화 추출(FetchedExam)이라 PDF 전체를 LLM이 재구조화하는 comcbt(FetchedFile)보다 품질·비용 모두 우위(번복이 아닌 확장 — qnet>comcbt 상대 순위 불변). 채택 안 된 어댑터는 대안 표기(also_on)만.
>   - **회차 동일성 판정 = 시험 날짜 자연 키(M12 갱신, 설계 §4.13)**: cbtbank에는 회차 번호가 없고 comcbt 라벨("YYYY년 MM월 DD일(N회)")에는 날짜·회차가 둘 다 있다 — 따라서 **시험 날짜(YYYY-MM-DD)가 있는 항목끼리는 날짜로 병합**하고, 날짜가 없는 항목은 기존 `YYYY-N` 키로 병합한다. 병합 그룹의 **대표 exam_key는 회차 번호 보유 키(`YYYY-N`) 우선**(분류 경로 "YYYY년 N회" 유지), 아무도 회차 번호를 모르면 날짜형 키(경로 "YYYY년 M월 D일"). 문서 단위 중복은 기존 반입 중복 감지(§8.3) 재사용 — 별도 장치 없음.
>   - **스키마 변경 불필요 근거(2026-07-25 확정 · M12 재검증 2026-07-26)**: 회차 메타는 기존 구조로 충분 — 회차 = `categories` 노드(분류 자동 생성 재사용), 문항 위치 = `documents.source_detail`("YYYY년 N회 M번"), 수집 출처 = `sources.note`(URL·어댑터 id). "이미 반입된 회차" 표시는 분류 경로 존재 여부에서 파생. 새 테이블·컬럼 없음. M12의 추가 정보(회차 시험 날짜·문항 과목 구분)도 **어댑터 자료구조(파이썬 dataclass)·병합·프롬프트 계층에서만 흐른다** — DB에는 기존 경로(분류 경로·source_detail·태그 제안)로만 반영, DDL 0건.
> - **원칙(설계 §4.13에 강제 조항으로 명문화)**: 개인 학습 전용·재배포 금지·요청 간격 제한(사이트별 **최소 2초**, robots 존중, 목록 캐시 24h). 로그인/CAPTCHA 필요 사이트는 범위 외(우회 코드 금지). 실행 전 예상 LLM 사용량(문항 수 기준) 안내 필수. 파싱 실패는 구조화 오류(`parse_failed`)로 표출하고 URL 반입(1단계)·대안 어댑터를 안내, 반입 후 오류는 F30 신고·재생성.
>
...
> 구현 순서는 `docs/01-plan/stage-1-skeleton.plan.md` ~ `stage-12-content-help.plan.md` 참조.
> MVP = Stage 1~3, v1 완성 = Stage 7 (2026-07-24), **v1.x = Stage 8~11 완료 (2026-07-26)**, v1.x 후속 = Stage 12(M12).
