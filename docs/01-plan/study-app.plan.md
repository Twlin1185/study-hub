...
> 상태: **확정 준비 (Draft v0.13)** — v0.12 대비: **M8~M11 구현·검토·머지 완료 표기 — v1.x 종료**(M8·M9 ✅ 07-25, M10·M11 ✅ 07-26). 기능·스키마 변경 없음 — 문서 최신화 정리(§14 로드맵 ✅·푸터, CLAUDE.md 문서 지도, 워크플로 스킬 문구, 사용자 매뉴얼 v1.x 전면 반영). S10 사후 수정 1건: comcbt 첨부 다운로드 3중 결함(URL 한글 인코딩·302 Location mojibake·PDF 매직 바이트 검사 — R14 실측 사례 추가). **다음 로드맵은 실사용 피드백 확인 후 이 계획서에 먼저 확정한다.** (v0.12: M11 계획·설계 확정 — F25·F16 상세 명세(§14)·설계 §4.14·R15, DDL 변경 없음.)
...
> v1.x는 위 로드맵 M8~M11로 확정 (2026-07-24, 시험 일정 무관 순차 진행 — 사용자 확정). **M8~M11 전 단계 구현·검토·머지 완료 — v1.x 종료 (2026-07-26).** 아래는 각 기능의 상세 명세. F15(문서 분기)는 실수요 확인 전 보류.
...
>   - **대상 사이트 2곳(2026-07-25 사용자 확정)**: **전자문제집 CBT(comcbt.com)** + **큐넷(Q-Net) 공개자료**. 어댑터 3호 이상은 실수요 확인 후.
>   - **중복 회차는 큐넷 우선(사용자 확정)**: 회차 키(`YYYY-N`) 정규화로 동일성 판정 — 양쪽에 있으면 큐넷(공단 원본 = 정본) 채택, comcbt는 대안 표기만. 문서 단위 중복은 기존 반입 중복 감지(§8.3) 재사용 — 별도 장치 없음.
>   - **스키마 변경 불필요 근거(2026-07-25 확정)**: 회차 메타는 기존 구조로 충분 — 회차 = `categories` 노드(분류 자동 생성 재사용), 문항 위치 = `documents.source_detail`("YYYY년 N회 M번"), 수집 출처 = `sources.note`(URL·어댑터 id). "이미 반입된 회차" 표시는 분류 경로 존재 여부에서 파생. 새 테이블·컬럼 없음.
...
> 구현 순서는 `docs/01-plan/stage-1-skeleton.plan.md` ~ `stage-11-exam-tools.plan.md` 참조.
