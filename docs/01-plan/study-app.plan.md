...
> 완료 마일스톤(M1~M16)의 원문 행(완료 서사·실측치)·결정 기록은 `docs/04-archive/roadmap-history.md`, 완료 기능 상세 명세(F16·F25·F26·F34~F42 — F35-2 제거 이력·F35-3 실측 확정 포함)는 `docs/04-archive/feature-specs-done.md`에 **원문 그대로** 보존돼 있다(2026-07-31 이동). 각 단계의 작업 지시서·완료 경위는 `stage-{n}-*.plan.md`가 단일 출처.
