...
- [ ] **B2-1** `DELETE /api/llm/jobs/{job_id}` (`routers/llm.py`) → `convert_service.dismiss_job(job_id)`: `_JOBS_LOCK` 아래 **종료(done·error·cancelled) 잡만** pop → `{status:'dismissed'}` · running/queued = 409 CONFLICT("진행 중인 작업은 취소 후 지울 수 있습니다") · 미존재 = 404
- [ ] **B2-1** `_job_ref`(convert) 에 `split_id` 추가(`{"preview_id", "split_id"}`) — 순수 추가
- [ ] **B2-2** `split_service` 조각 라벨에서 `/` 제거: `f'{label} ({idx+1}/{pieces})'` → `f'{label} ({idx+1}-{pieces})'`, 균등 분할 라벨 동일 · `_normalize_chunks`·LLM 정제 라벨도 `/` → `-` 치환(라벨은 분류 접미 초안이므로 경로 구분자를 포함하면 안 된다)
- [ ] **B2-2** `enqueue_split`: `category_paths` 전부를 잡 등록 **루프 이전에** `normalize_category_path`로 검증(부분 등록 후 422 방지)
- [ ] **B2-2** `POST /api/import/preview/merge` (`routers/imports.py`) body `{preview_ids: [str,…]}`(2개 이상 · 각각 `_PREVIEW_CACHE` 존재 또는 보존본 복구 가능) → 각 preview의 정규화 문서(`items[i]["doc"]`)를 **주어진 순서로 연결**한 `{"documents":[…]}`로 `create_preview(preserve=True, warnings_override=<재인덱스한 조각 경고>, source_filename="<첫 조각 파일명> (분할 병합 N조각)")` 호출 → `PreviewResponse` 그대로 응답. 누락·만료 preview = 404(어느 id인지 detail). 원 조각 preview는 삭제하지 않는다(TTL 자연 만료)
- [ ] **B3** `schemas/import_schema.py` `PreviewItem`에 `content: Optional[str]`, `choices: Optional[list]`, `answer: Optional[str]`, `explanation: Optional[str]` (모두 기본 None) → `create_preview`에서 `norm`으로 채움(정규화 문서의 키 이름은 `import_service` 정규화 결과 기준 — 없는 키는 None)
- [ ] **B3** `ImportDecision.override: Optional[ItemOverride]` (`title?`, `content?`, `answer?`, `explanation?` — 모두 Optional str) → `commit_import`에서 해당 항목 `doc`에 **얕은 덮어쓰기 후** 기존 new/merge 경로 진행. 빈 문자열 title은 422("제목은 비울 수 없습니다")
- [ ] **B4-S4/S6** `convert_service.normalize_category_path`의 본체를 **관대 정규화기**로 분리(`normalize_category_path_lenient(path) -> Optional[str]`: `>`·`＞`·`»`·`≫`·`\`·`::` → `/` 치환 → 세그먼트 strip·NFC → 빈 세그먼트 제거 → 5단·60자 초과면 None) — 기존 엄격 함수는 동작 불변(사용자 입력 422 유지)
- [ ] **B4-S7/S1** `import_service._validate_item`: `suggest_categories`가 list가 아니거나 원소가 str이 아니면 **항목 error가 아니라** str 원소만 회수(dict면 `path` 키) + warning `'category_malformed'`; 각 경로는 관대 정규화기 통과(None이면 버림). 제안이 0개면 warning `'no_category'`
- [ ] **B4-S5** `import_service._find_child`·`_path_names`: 이름 비교를 `unicodedata.normalize('NFC', name).strip()` 기준 + 대소문자 무시(`func.lower`)로 — 생성 시에는 원문(정규화·strip) 그대로 저장. 커밋 `_apply_categories`의 문자열 경로도 관대 정규화기 통과
- [ ] **B4-S2/S3** `improve_service.load_convert_prompt_with_casebook`: `prompts/taxonomy.md` 첨부("## 부속 분류 정책"). `convert_service._category_directive_lines`: "이 지시가 분류 정책의 2경로 규칙보다 우선한다 — 다른 경로를 추가하지 말 것" 1줄 추가
- [ ] 테스트: `tests/test_import_categories_lenient.py`(신규 — S4·S5·S7·no_category 경고) · `tests/test_preview_merge.py`(신규 — 2 preview 병합 재인덱스·경고 승계·404) · `tests/test_job_center.py`에 dismiss 3케이스 추가 · 기존 전체 `run-tests.ps1` 통과

### 프론트 (`frontend-dev` · sonnet)

- [ ] **B2-1** `api/llm.ts` `dismissJob(jobId)` → `Import.tsx` `onSplitStarted`(split 시작 성공 시) 앵커 항목의 `entry.jobId`가 있으면 호출(실패는 무시·콘솔 경고만) + `llmJobsKey` 무효화. `JobCenterPanel.tsx` 종료 잡 카드에 [목록에서 지우기] 버튼
- [ ] **B2-2** `SplitImportWizard.tsx`: `initialCommonPath?: string` prop(공통 경로 초기값) · 조각 `pathSuffix` 기본값 **`''`**(라벨은 표시만 — "접미 추가" 입력은 그대로 두되 비어 있으면 공통 경로만 사용) · `mergeWithNext`는 `a.pathSuffix` 유지 · 접미/라벨의 `/`는 `-`로 치환. `Import.tsx` `handleSplitImport`에서 `item.entry.categoryPath`를 전달
- [ ] **B2-2** `useConvertQueue.addJobs`: 생성 항목에 `splitId`(인자) · `splitTotal: jobs.length` 보존(현행 `splitId: null` 정정 — 앵커 제거 로직은 **앵커(`sourceKind !== 'split'`)만** 대상으로 좁힌다). `ImportQueue.tsx`: 같은 `splitId` 조각을 한 그룹 헤더("분할 조각 k/N 검토 대기")로 묶고, 그룹의 **모든 조각이 종료**(ready·error)이고 ready ≥ 2면 [합쳐서 검토] → `api/imports.ts` `mergePreviews(previewIds)` → 응답 preview로 **새 항목 1개**(`sourceKind:'split'`, `jobId:null`, `previewId`, `fileName: "<원본> (분할 병합 N조각)"`, `categoryPath`= 조각 공통 경로) 적재 + 병합된 조각 항목 제거 → 바로 검토 단계 진입. 실패 조각은 그룹에 남긴다. 큐 상태 산출이 `jobId:null && previewId`를 `ready`로 취급하는지 확인·보정
- [ ] **B3** `api/types.ts` `ImportItem`에 `content?/choices?/answer?/explanation?` · `ImportDecision.override?`. `Import.tsx` `ItemRow`: [본문 보기] 토글(접힘 기본) → `MarkdownView`로 본문·선택지(번호 목록)·정답·해설 렌더 · [편집] 토글 → 제목 `input` + 본문/정답/해설 `textarea`(정답·해설은 type이 문제류일 때만) · 변경분은 `decisions[index].override`에 보관 · 편집된 항목은 "편집됨" 배지 · commit 요청에 `override` 포함(변경 없으면 생략). 색상은 토큰만
- [ ] **B4-S1/S8/S12** `Import.tsx`: 분류 블록 **항상 렌더**(제안 0 = "분류 제안 없음 — 아래에 경로를 입력하면 생성·연결합니다") + 항목별 `CategoryPathField`(또는 동일 검증의 단순 입력)로 **경로 추가** → `approvedCategoryIds`에 문자열로 추가(기존 계약 — `str` = 생성 경로). `buildInitialDecisions`에서 `container === true` 제안은 기본 미체크. 경고 배지에 `'no_category'`·`'category_malformed'` 문구 추가. `CategoryPathField.tsx:88` 안내 문구를 실제 동작("해제·추가")에 맞게 정정
- [ ] `npm run build` 성공 · 초기 청크 증가 ≤ 10KB(min) 확인
