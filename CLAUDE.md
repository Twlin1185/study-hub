...
| `docs/02-design/study-app.design.api.md` | §4 API 명세(4.1~4.19, [S1]~[S17] 태그) — 필요한 절만 `### 4.<n>` Grep 후 부분 읽기 |
...
| `docs/01-plan/stage-{1..17}-*.plan.md` | 단계별 작업 지시서 — 체크리스트·DoD·제외 범위. **stage 1~17 완료**(17은 2026-08-02 검토 통과·경미 결함 후속 수정까지 완료 — 경위는 stage-17 문서 완료 기록 참조) · **18**(해설·정답 보완 F44)·**19**(응용 모의고사 F45, M19)·**20**(반입 자기 개선 루프 F46, M20)은 계획서 등재만(18 착수 가능 — 게이트 없음, 19는 F44 선행 권장, 20은 순서 자유). 각 단계의 완료 경위·게이트 기록은 해당 stage 문서 참조 |
