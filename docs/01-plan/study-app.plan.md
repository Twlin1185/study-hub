...
| **M38. 통합 휴지통(F61 — 별지 D8-구현) → v2.03.1 예정** | 문서·노트 소프트 삭제분 목록 + 복원 · 고아 이미지 식별 → `sources/images/.trash/` 이동·되돌리기(앱 자동 삭제 0 · 최종 삭제 = 사용자 수동) · 불변 규칙 4 개정(§6.3 — 업로드·수집 이미지 = 파생물 · 반입 원본 불변 유지) · 신규 엔드포인트 7 · DDL 0 — 지시서 `stage-52-trash-orphan-images.plan.md` | 편성 2026-09-14 · 착수 전(핵심 · FB-24 ⓒ는 이 뒤 별도) |
