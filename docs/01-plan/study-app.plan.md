...
> 상태: **확정 준비 (Draft v0.4)** — 다크 모드(F28)·북마크(F29) 채택, 설계 문서(`docs/02-design/study-app.design.md`)·단계별 구현 계획(stage 1~6) 연결
> 작성일: 2026-07-22
...
| F16 | 시험 D-Day 모드 | 시험일 등록 → 남은 기간 기반 복습 강도 조절 |
...
| **M4. 시각화·인쇄** | 대시보드(차트·히트맵), 문서 사용처·문제↔개념 연결 표시(F24), 북마크(F29), A4 인쇄 뷰 3종 | 약점이 한눈에 보이고, 문제집 PDF 출력 가능 |
| **M5. 복습** | SM-2, 오늘의 복습 큐, 플래시카드 | 매일 복습 루틴 동작 |
| **M6. 자동화·다듬기** | 태그 자동분류, claude CLI 연동 변환, 검색(FTS5), PWA, 백업/복원 UI(F27) | 반입~분류가 앱 안에서 완결 |
