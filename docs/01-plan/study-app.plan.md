...
> - **④ 변환 출력 잘림 오안내 수정**: JSON 파싱 실패가 `kind:'other'` + **"잠시 후 다시 시도하세요"** 로 안내돼(같은 파일로 재시도하면 같은 실패) 원인·해결책 없이 LLM 비용만 반복 지출된다 — 문항이 많으면 출력 토큰 상한에서 잘리는 사례가 이미 코드 주석에 기록돼 있다. → `error_info.kind:'invalid_output'` 신설: "응답이 중간에 잘렸을 수 있습니다 — 원본을 과목·회차 단위로 나눠 올리거나 다른 엔진으로 재시도". **서버측 PDF 분할은 하지 않는다**(라이브러리 의존 금지 — F35-3 비지원 포맷 정책과 같은 원칙). 원문(raw) 노출 금지 유지.
...
> MVP = Stage 1~3, v1 완성 = Stage 7 (2026-07-24), **v1.x = Stage 8~11 완료 (2026-07-26)**, v1.x 후속 = Stage 12(M12, ✅ 2026-07-26) · **Stage 13**(M13 — 사설 어댑터 제거 + 수동 반입 UX F40, `stage-13-fetch-cleanup-manual-import.plan.md`) · **Stage 14**(M14 — 큐넷 오픈API, 후순위, `stage-14-qnet-openapi.plan.md`). 구 `stage-13-qnet-openapi.plan.md`는 두 문서로 분할된 표지만 남아 있다(2026-07-27).
