...
> 상태: **확정 준비 (Draft v0.11)** — v0.10 대비: **M10 계획·설계 확정** — F35 2단계 사용자 확정 반영(어댑터 대상 **comcbt+큐넷 공개자료 2곳**, 중복 회차 **큐넷 우선** — 2026-07-25), F26 상세 명세 신설(§14 — 목표는 settings 키·스트릭은 파생값, **스키마 변경 불필요 근거 확정**), F35 2단계 회차 메타도 기존 categories·source_detail·sources로 충분(DDL 없음), 리스크 R14(사이트 구조 변경) 추가, Stage 10 작업 지시서(`stage-10-content-motivation.plan.md`)·설계 §4.13 연결. (v0.10: M9 계획·설계 확정.) **스키마(§6.2 DDL) 변경 없음 — Alembic 마이그레이션 0건**
...
> 구현 순서는 `docs/01-plan/stage-1-skeleton.plan.md` ~ `stage-10-content-motivation.plan.md` 참조. MVP = Stage 1~3, v1 완성 = Stage 7, v1.x = Stage 8~.
