...
> **상태: 착수 전 결정 ①~⑦ 전건 확정(2026-08-02) — 착수 가능.**
...
- [x] stage-reviewer(Opus) 검토: DoD + 자동 반영 경로 0 + 코드 자기 수정 경로 0(쓰기 경로 전수 = improve/·prompts/ 한정) + 사용량 안내 없는 LLM 호출 0 + DB 쓰기 0 + 수집 best-effort(원 플로우 무영향) — **2026-08-02 조건부 통과**(DoD 자동 검증 6/6·치명 0·중요 1(dist 미재빌드)은 당일 재빌드로 해소·경미 7건은 완료 기록에 목록 보존 — 수정 여부 사용자 결정 대기)
...
- **검토 경미 7건(미해소 — 수정 여부 사용자 결정 대기)**: ② 잡 상태 타입 `'prepared'` 누락(schemas·프론트 types — stage-19 경미 ⑤ 동일 유형, 런타임 영향 없음) ③ improve 잡 `invalid_output` 안내 문구가 반입용 기본값(사례 축소·교체 안내가 맞음) ④ 폐기 사유·회귀 outcome 영문 코드 노출(백엔드 한국어 라벨 미사용·labels.ts 미사용 1곳) ⑤ 사례 선택 20건 상한 UI 미반영(21건 시 pydantic 422 영문 detail) ⑥ 회귀 판정이 원문 대조 불가(정규화 <200자)를 `passed`로 기록(R23 ③ 수치 신뢰 저하 — `unavailable` 처리 권고) ⑦ hash12 없는 사례(FetchedExam 경로) 과대 병합(kind별 1건으로 합쳐짐 — 키에 preview_ref/exam_key 포함 권고) ⑧ "DB 쓰기 0"의 예외 1경로(`rate_limit` 시 공용 `remember_limit`가 settings 기록 — 문서 병기 권고).
- **사용자 확인 항목**(라이브 루프 완주 DoD 7 · 제안 품질 표본 DoD 8 — 결과 수치): **대기** — 완료 시 여기와 plan F46·R23에 기록.
- **테스트 최종**: pytest **391 passed**(신규 test_improve.py 41건 — 검토자 재실행 2회 확인) · 프론트 `npm run build` 타입 에러 0 · dist 재빌드 커밋(index-Bsk8ZRB5.js).
