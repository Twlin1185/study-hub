...
- [x] stage-reviewer(Opus) 검토: DoD + 격리 전수(일반 출제·이력·통계 혼입 0) + 응시 응답 계약 + 사용량 안내 없는 LLM 호출 0 + 트랜잭션(규칙 2) — **2026-08-02 통과**(DoD 자동 검증 6/6·치명 0·중요 1(basis 원소 타입 미방어)은 수정 후 표적 재검토로 해소·pytest 344. 경미 6건은 완료 기록 참조 — 수정 여부 사용자 결정)
...
- **검토 잔여(경미 6건 — 수정 여부 사용자 결정)**: ① prepare 범위에 applied 서브트리 제외 없음(자기참조 생성 가능 — 서버측 차집합 권고) ② SourceMatcher 10자 미만 후보의 "생략=True" 반전 미가드(실무 영향 거의 없음) ③ `invalid_output` 안내 문구가 convert용 오안내(S18부터 있던 갭) ④ 통과 0건 실패 시 폐기 사유 미도달(메시지 합성 권고) ⑤ 프론트 status 타입에 'prepared' 누락 ⑥ (해소) dist 재빌드 — 완료 커밋에 포함. **문언 긴장 1**: 실전 탭에서 applied 분류를 명시 선택해 응시하면 mode='exam'으로 실전 이력에 들어가는 **옵트인 경로 1개 잔존**(§4.21이 옵트인 예외로 수용한 경로 — F25 계약 불변이라 코드 수정 안 함).
- **사용자 확인 항목**(라이브 완주 DoD 7 · 표본 검증 DoD 8 — 결과 수치): **대기** — 완료 시 여기와 plan F45·R22에 기록.
- **테스트 최종**: pytest **344 passed**(신규 24 = 본 구현 20 + 재검토 수정 4) · `npm run build` 통과(TS 오류 0) · 스모크 전 항목 정상.
