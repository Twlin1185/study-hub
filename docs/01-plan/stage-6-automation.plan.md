...
> 상위: `study-app.plan.md` v0.5 §11, §13 · 설계: `../02-design/study-app.design.md` §4.9, §4.10, §5.3(오류 신고·재생성)
> 선행: Stage 2(반입)·Stage 4 권장 · 포함 기능: F12, F17, F18, F21, F23, F27, F30

## 목표

반입→분류→검색이 앱 안에서 완결. "파일을 던지면 변환·반입·분류 제안까지" 자동화.
문제 오류는 신고 한 번으로 재생성→비교→교체까지(F30).
...
6. 문제 화면에서 오류 신고 → 재생성 → 기존/신규 비교 → [교체] 후 같은 문서 번호로 갱신되고 풀이 이력·오답노트가 유지됨 (승인 없는 자동 덮어쓰기 없음)
