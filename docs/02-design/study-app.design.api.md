...
구현 단계 표기: [S1]~[S28] = stage 1~28에서 구현. (S7은 순수 프론트 단계 — 새 엔드포인트 없음, 기존 settings API의 키 추가만. S26·S27은 프론트 전용 — §4 무변경. S28은 신규 엔드포인트 0개 — documents PATCH `style` 필드 + settings 키 추가, §4.26.)
