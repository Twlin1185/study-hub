...
> (v1.65: v1.64 대비: **api §4.2·§4.28 ① `[S52]` 복원 행 2개 + `### 4.32 [S52]` 절 신설 + §4.27 말미·§4.28 ⑥ `← S52` 추기 + screens `### 5.17 휴지통` 신설 + §5.11 휴지통 카드·§5.2·§5.3·§5.16 삭제 문구 S52 1줄 4곳(stage-52 통합 휴지통 — 문서·노트 복원 + 고아 이미지 2단계 정리 편성, 2026-09-14 — 문서만·코드 0·**착수 전** · 실측 재개정은 완료 시 v1.66)**: ① **신규 엔드포인트 7** — `GET /api/trash/documents`·`/notes`(`is_active=0`만 · `updated_at DESC`) · `POST /api/documents/{id}/restore`·`POST /api/notes/{id}/restore`(`is_active=1` UPDATE만 · 멱등 · 링크·태그·학습 기록 무접촉) · `GET /api/trash/images?min_age_days=7`(고아 스캔 보고서 — 읽기 전용 · 참조원 = documents 5컬럼 + notes 2컬럼(활성+비활성 전 행) + `import/auto/*.json` 문자열 정규식 · 7일 유예) · `POST /api/trash/images/move`·`/restore`(`sources/images/` ↔ `sources/images/.trash/` · `shutil.move` · 파일명 정규식 fullmatch + 루트 종속 검사 · 이동 직전 참조 재검사 422 `still_referenced` all-or-nothing · **삭제 코드 0 · 최종 삭제 = 사용자 수동**) ② **screens §5.17** — 독립 라우트 `/trash` 탭 3개(문서·노트·이미지) · 진입 = 설정 ⑤ 데이터 그룹 카드 1개(사이드바·드로어 무접촉 · F38 6그룹 불변) · 스캔 수동 · 썸네일 = 기존 서빙 ③ **불변 규칙 4 개정 동반**(계획서 §6.3 4 + `CLAUDE.md` — `sources/images/` 업로드·수집 이미지 = 파생물 · 반입 원본 불변 유지). **DDL 0(`deleted_at` 없음 — `updated_at` 대용) · Alembic 0 · settings 키 0 · 신규 의존 0 · 버전 영향 핵심(v2.03.1 예정).** 정본 = `stage-52-trash-orphan-images.plan.md` §2.)
> (v1.64: v1.63 대비: **api §4.31 `[S51]` 구현 실측 확정 표기(절 머리 + §4.2 행) + 실측 확정 ⓐ 추기(`move` 행) + screens §5.2·§5.3 S51 실측 재개정 2곳(stage-51 탐색 문서 다중 선택 + 일괄 도구 구현·검토 완료 반영, 2026-09-13 — 문서만·API 계약 그대로 구현 · 구현 커밋 07c6d13 + 검토 반영 7aabd4d · 백엔드 diff = `routers/documents.py`·`services/document_service.py`·`schemas/document.py`·`tests/test_documents_bulk.py`(22건) · pytest 659 · invariant PASS(physical-delete 기준선 4→7 = `category_documents` 링크 행 삭제 규약 D·E 정당분 · `documents` 무접촉) · `npm run build` 성공 · Opus 검토: 1차 재작업(중요 1·경미 3) → 반영 후 재검토 **통과**(치명·중요·경미 0) · 잔여 = 브라우저 V-3(사용자 서버 재시작 후) + 사용자 실사용 게이트 DoD 6 · **v2.02.1 발행 대기(stage-48·49·50 v2.01.3와 동시 발행)**)**: ① **api 실측 확정 ⓐ** — `move`의 대상 집합은 출발 하위 트리 **− {도착}**(`deep`이고 도착이 출발 하위면 종전 정의로는 도착 행 자신까지 삭제돼 문서 고아화 — 검토 중요-1 · 도착에만 연결된 문서 = `skipped` · `study_progress` 이관도 같은 집합) ② 분류 존재 검사 = action별 관련 필드만(delete = 검사 0 — 규약 A "무시" 그대로 · 검토 경미-2) ③ **screens 실측 정정** — 폴백 문구 호출처 5곳 `e.message`(mutationFn 1곳 규약 복원 · 검토 경미-1) · 체크박스 ⓐ 상시 노출 확정. **DDL 0 · Alembic 0 · 신규 엔드포인트 1 · 신규 의존 0 · 버전 영향 핵심.** 정본 = `stage-51-explore-bulk-select.plan.md` §7.)
